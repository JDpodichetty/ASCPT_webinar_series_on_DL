function preprocessBraTSdataset1(destination,source)
...
if ~exist(volLoc,'dir') || ~exist(lblLoc,'dir')
    error(['Please unzip Task04_Hippocampus.tar file to ' source])
end
...
if ~exist(destination,'dir') || proceedWithPreprocessing(destination)
    ...
    while hasdata(pxds)
        ...
        if (id < floor(0.83*NumFiles))
            imDir    = fullfile(destination,'imagesTr','HippoTS');
            labelDir = fullfile(destination,'labelsTr','HippoTS');
        elseif (id < floor(0.89*NumFiles))
            imDir    = fullfile(destination,'imagesVal','HippoTS');
            labelDir = fullfile(destination,'labelsVal','HippoTS');
        else
            imDir    = fullfile(destination,'imagesTest','HippoTS');
            labelDir = fullfile(destination,'labelsTest','HippoTS');
        end
